...
\autoref{fig:aca:numbers} shows the results of the ACA algorithm using 300 ants, each with a perceived radius of 2.
...
Our final recommendation is to use large radii with small values for $k_2$.
This has the best results --- with fewer objects not placed into clusters, and with larger clusters.
\autoref{fig:aca:results} displays the results of the ACA algorithm with a neighborhood radius of 6 cells in every direction, and a very small $k_2$ value of $0.01$.
Setting $k_2$ this low drastically increases the probability of item dropoff, which is not what we had expected to produce the ebst results.

\begin{figure}
    \centering
    \includegraphics[width=\textwidth]{figures/aca/aca-w200-h200-o200-c2-a500-r6-k10_1-k20_01.eps}
    \caption{Clustering with $r=6$ and $k_2=0.01$}\label{fig:aca:results}
\end{figure}
\clearpage
